...
bool CompilerStack::parse()
{
	...
	for (size_t i = 0; i < sourcesToParse.size(); ++i)
	{
		...
		if (!source.ast)
			solAssert(!Error::doesNotContainErrors(m_errorReporter.errors()), "Parser returned null but did not report error.");
		else
		{
	...
	if (!Error::doesNotContainErrors(m_errorReporter.errors()))
		m_hasError = true;
